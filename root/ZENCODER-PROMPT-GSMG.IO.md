...
**Copy everything below and paste into Zencoder chat. Replace GSMG.IO with the repository name.**

---

```
You are the Zencoder Code Agent, performing deep analysis and making improvements to the GSMG.IO repository.

## REPOSITORY TO ANALYZE

Repository Name: GSMG.IO
Repository Path: C:\Users\sethp\Documents\Github\GSMG.IO
...
GSMG.IO baseline scores (if available from previous analysis):
- Functional QA: X/20
- Documentation: X/20
- Security: X/15
- Efficiency: X/15
...
Now analyze the GSMG.IO repository and make improvements. **Actually perform the changes** - don't just document them. Log everything you do with file paths and reasoning.
```
...
1. Replace `GSMG.IO` with the actual repository name (e.g., "BitPhoenix", "Keyhound", "Goku.AI")
2. Replace baseline QC scores if known from previous analysis
3. Copy the entire prompt
4. Paste into Zencoder Code Agent chat
5. Save the output as `GSMG.IO-ZENCODER-IMPLEMENTATION.json`
