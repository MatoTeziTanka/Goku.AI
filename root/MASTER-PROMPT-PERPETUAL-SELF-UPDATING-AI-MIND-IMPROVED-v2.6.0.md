...
## TIER 1: QUICK START GUIDE (Daily Reference)

**Purpose:** Essential rules for day-to-day operations. Full framework details in subsequent sections.

### 🚨 10 ABSOLUTE MUST-FOLLOW RULES

1. **Issue-Driven Workflow (MANDATORY)**
   - Every task MUST start with a GitHub issue
   - Use "To-Do" label for active work
...
3. **Credit Balance Monitoring (MANDATORY)**
   - Check Azure/AWS balance BEFORE every API call
   - Check balance AFTER every API call
   - Log balance changes
   - **Reference:** Appendix B, C

4. **Logging (MANDATORY)**
   - Use Who/What/Where/When/Why format
   - Log to SSD for active sessions
   - Archive to HDD for long-term storage
   - **Reference:** Section 14

5. **Code Quality (MANDATORY)**
   - Minimum 95/100 QC score
   - 80% code coverage (with documented exceptions)
   - All functions must have docstrings
   - **Reference:** Section 9

6. **Storage Paths (MANDATORY)**
   - Use centralized `STORAGE_CONFIG` (Section 19.2)
...
7. **Error Handling (MANDATORY)**
   ...
   - Log error severity (CRITICAL → LOW)
   - Follow self-correction protocol
   - **Reference:** Section 23

8. **State Management (MANDATORY)**
   - Save checkpoints to SSD before long operations
   - Archive completed work to HDD
   - Reconstruct state from checkpoints on restart
   - **Reference:** Section 19.2, 19.3

9. **Agent Collaboration (MANDATORY)**
   - Skeptical Reviewer has veto power
   - All agents must identify themselves
   - Log agent handoffs
   - **Reference:** Section 4
...
```
1. Create GitHub Issue → Label: "To-Do"
2. Assign to agent/yourself
3. Work on issue
4. Create/update code with security scanning
5. Log all operations (Who/What/Where/When/Why)
6. Check credit balances (before/after API calls)
7. Update issue with progress
8. Close issue when complete
9. Archive logs to HDD
```

**Full Details:** Section 18.0
...
**Full Details:** Section 3.2

### 💰 CREDIT MONITORING (Quick Reference)

```python
# Before Azure API call:
balance_before = azure_credit_monitor.log_balance_before("operation_name")
result = azure_api_call()
balance_after = azure_credit_monitor.log_balance_after("operation_name", balance_before)
```

**Full Details:** Appendix B, C

### 📊 STORAGE PATHS (Quick Reference)

```python
# Use centralized config (Section 19.2):
from config import STORAGE_CONFIG, SSD_STORAGE, HDD_STORAGE

# Active work → SSD
log_file = SSD_STORAGE / "session_logs" / "current.json"

# Archive → HDD
archive_file = HDD_STORAGE / "memory_stash" / "archive.json"
```

**Full Details:** Section 19.2
...
**Need help?**
- ✅ Check Cross-Reference Index (Section 25)
- ✅ Review relevant section
- ✅ Check Quick Reference (this section)

---

**Note:** This is a condensed reference. For complete details, see the full framework sections referenced above.
...
**NOTES:**
- **NOTE:** Regex patterns for secret detection are provided for reference only; always use robust libraries for implementation.
...
#### 3.4.1 Multiprocessing-Safe Logger

**For parallel repository processing and data processing workloads:**
...
class ProcessSafeLogger:
    """Logger safe for both threading and multiprocessing."""
    
    def __init__(self, log_file: Path, buffer_size: int = 100):
        """
        Initialize process-safe logger.
        ...
        # Use Manager for process-safe shared state
        self.manager = mp.Manager()
        self.buffer = self.manager.list()
        self.lock = self.manager.Lock()  # Process-safe lock
        self.counter = self.manager.Value('i', 0)  # Process-safe counter
        
        # Ensure log directory exists
        self.log_file.parent.mkdir(parents=True, exist_ok=True)
    
    def log(self, who: str, what: str, where: str, why: str, status: str = "INFO", **kwargs):
        """
        Log entry (process-safe).
        
        Args:
            who: Who performed the action
            what: What action was performed
            where: Where it happened
            why: Why it was performed
...
# Usage in multiprocessing context:
def process_repository_parallel(repos: list[Path], num_workers: int = 4):
    """Process multiple repositories in parallel."""
    from config import SESSION_LOGS
    
    # Create process-safe logger
    logger = ProcessSafeLogger(SESSION_LOGS / "parallel_processing.log")
    
    def process_single_repo(repo_path: Path):
        """Process a single repository (runs in worker process)."""
        logger.log(
            who="process_repository_parallel",
            what="start",
            where=str(repo_path),
            why="Parallel processing: Starting repository",
            status="INFO"
        )
        ...
        logger.log(
            ...
            what="complete",
            where=str(repo_path),
            why="Parallel processing: Repository complete",
            status="SUCCESS",
            result=result
        )
    ...
    with mp.Pool(num_workers) as pool:
        results = pool.map(process_single_repo, repos)
...
- ✅ Data processing workloads with multiple workers
- ✅ Any scenario with `multiprocessing.Pool` or `Process`

**When NOT to Use:**

- ❌ Single-threaded operations (use `BufferedLogger` instead)
- ❌ Simple scripts without parallel processing
- ❌ When performance overhead is unacceptable

**Performance Notes:**

- Process-safe logging has higher overhead than thread-safe logging
- Use `BufferedLogger` for single-process operations
- Use `ProcessSafeLogger` only when multiprocessing is required
...
### 19.2.1 Centralized Storage Configuration (Single Source of Truth)

**CRITICAL:** All storage paths must use this centralized configuration. Never hardcode paths.

**Create `config.py` in project root:**

```python
# config.py - Single Source of Truth for Storage Paths
import os
from pathlib import Path
...
def validate_storage_paths():
    ...
    Raises:
        ValueError: If storage parent directories don't exist
    """
...
# Define constants (use these throughout the framework)
...
from config import SSD_STORAGE, HDD_STORAGE, SESSION_LOGS, MEMORY_STASH

log_file = SESSION_LOGS / f"session_{session_id}.json"
archive_file = MEMORY_STASH / category / f"{datetime.now().strftime('%Y%m%d')}.json"

# ❌ WRONG - Never hardcode paths
# log_file = Path(r"C:\AI_Storage\SSD\session_logs\session.json")  # DON'T DO THIS
```
...
```bash
# Windows PowerShell
$env:AI_STORAGE_SSD="C:\Custom\SSD\Path"
$env:AI_STORAGE_HDD="D:\Custom\HDD\Path"

# Windows CMD
set AI_STORAGE_SSD=C:\Custom\SSD\Path
set AI_STORAGE_HDD=D:\Custom\HDD\Path

# Linux/Mac
export AI_STORAGE_SSD="/custom/ssd/path"
export AI_STORAGE_HDD="/custom/hdd/path"
```

**All references to storage paths throughout this document use these constants.**
...
def calculate_timeout(repo_path: Path) -> int:
    """
    Calculate tool execution timeout based on repository size and complexity.
    
    Dynamic timeout calculation:
    - Base timeout: 60 seconds
    - Additional time: 1 second per 10MB of repository size
    ...
    Returns:
        Timeout in seconds (60-600 range)
    """
    try:
        ...
        # Dynamic timeout: base 60s + 1s per 10MB
        calculated_timeout = int(60 + (repo_size_mb / 10))
        
        # Clamp between 60s (minimum) and 600s (maximum)
        return max(60, min(600, calculated_timeout))
        
    except Exception:
        return 300  # Default fallback on error

# Timeout override mechanism (for special cases)
def get_timeout_with_override(repo_path: Path, override: int = None) -> int:
    """
    Get timeout with optional override.
    
    Args:
        repo_path: Path to repository
        override: Optional timeout override in seconds
        
    Returns:
        Timeout in seconds
    """
    if override is not None:
        return max(60, min(600, override))  # Clamp override
    return calculate_timeout(repo_path)
...
## Appendix B: Azure Free Tier Account & Credit Monitoring

**Account Status:** Free services for 12 months  
**Expires:** Tuesday, October 6, 2026  
**Last Updated:** 2025-11-26  
**Current Usage:** 0% across all services (Not in use)

### MANDATORY: Credit Balance Monitoring

**CRITICAL REQUIREMENT:** Always check Azure credit balance **BEFORE** and **AFTER** any Azure API call.

#### Step 1: Find Billing Profiles
...
def get_billing_profiles(access_token):
    """
    Get list of billing accounts and their billing profiles.
    
    Returns:
        List of billing profiles with their IDs
    """
    url = "https://management.azure.com/providers/Microsoft.Billing/billingAccounts"
    params = {
...
```python
def get_credit_balance(billing_account_id, billing_profile_id, access_token):
    """
    Get current credit balance for a billing profile.
    ...
    """
    url = f"https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billing_account_id}/billingProfiles/{billing_profile_id}/providers/Microsoft.Consumption/lots"
    ...
    for lot in data.get("value", []):
        ...
        closed_balance = props.get("closedBalance", {}).get("value", 0.0)
        
        # Only count active credits (not expired)
        expiration_date = props.get("expirationDate")
        if expiration_date:  # Check if not expired
            active_credits.append({
                "originalAmount": props.get("originalAmount", {}).get("value", 0.0),
                "closedBalance": closed_balance,
                "source": props.get("source", ""),
                "startDate": props.get("startDate", ""),
                "expirationDate": expiration_date
            })
            total_balance += closed_balance
...
#### Step 3: Get Credit Transactions

```python
def get_credit_transactions(billing_account_id, billing_profile_id, access_token, start_date, end_date):
    """
    ...
    """
    url = f"https://management.azure.com/providers/Microsoft.Billing/billingAccounts/{billing_account_id}/billingProfiles/{billing_profile_id}/providers/Microsoft.Consumption/events"
    params = {
...
#### Complete Credit Monitoring Class
...
class AzureCreditMonitor:
    """Monitor Azure credit balance before and after API calls."""
    ...
    def __init__(self):
        ...
        self._initialize_billing_info()
    
    def _initialize_billing_info(self):
        """Initialize billing account and profile IDs."""
        # Get access token
        self.access_token = self.credential.get_token("https://management.azure.com/.default").token
        
        # Get billing profiles
        profiles = get_billing_profiles(self.access_token)
        
        # Find the appropriate billing profile (you may need to customize this)
        for account in profiles.get("value", []):
            for profile in account.get("properties", {}).get("billingProfiles", []):
                # Use the first available profile or match by displayName
                if profile.get("properties", {}).get("displayName") == "Development":
                    self.billing_account_id = account.get("id").split("/")[-1].split(":")[0]
                    self.billing_profile_id = profile.get("id").split("/")[-1]
                    break
    
    def get_current_balance(self):
        """Get current credit balance."""
        if not self.billing_account_id or not self.billing_profile_id:
            raise ValueError("Billing account/profile not initialized")
    ...
    def log_balance_before(self, operation_name):
        """Log balance before Azure API operation."""
        balance = self.get_current_balance()
        
        logger.log(
            who="AzureCreditMonitor",
            what="check_balance_before",
            where="Azure Billing API",
            why=f"Credit monitoring: Before {operation_name}",
            status="INFO",
            balance_before=balance["total_balance"],
            currency=balance["currency"],
            active_credits_count=len(balance["active_credits"])
        )
        
        return balance
    
    def log_balance_after(self, operation_name, balance_before):
        """Log balance after Azure API operation."""
        balance_after = self.get_current_balance()
        balance_change = balance_after["total_balance"] - balance_before["total_balance"]
        
        logger.log(
            who="AzureCreditMonitor",
            what="check_balance_after",
            where="Azure Billing API",
            why=f"Credit monitoring: After {operation_name}",
            status="INFO",
            balance_before=balance_before["total_balance"],
            balance_after=balance_after["total_balance"],
            balance_change=balance_change,
            remaining_balance=balance_after["total_balance"],
            currency=balance_after["currency"]
        )
        
        # Alert if balance is low
        if balance_after["total_balance"] < 10.0:
            logger.log(
                who="AzureCreditMonitor",
                what="low_balance_alert",
                where="Azure Billing API",
                why="Credit monitoring: Low balance warning",
                status="WARNING",
                remaining_balance=balance_after["total_balance"]
            )
...
# Global instance
azure_credit_monitor = AzureCreditMonitor()
```

#### Usage Pattern (MANDATORY for All Azure API Calls)

```python
def azure_api_call_with_balance_check(operation_name, azure_function, *args, **kwargs):
    """
    Wrapper function to ensure balance is checked before and after Azure API calls.
    
    Args:
        operation_name: Name of the operation (for logging)
        azure_function: The Azure API function to call
        *args, **kwargs: Arguments to pass to azure_function
        
    Returns:
        Result of azure_function
    """
    ...
    try:
        ...
        result = azure_function(*args, **kwargs)
        ...
        # Still check balance even if operation fails
        try:
            azure_credit_monitor.log_balance_after(operation_name, balance_before)
        except:
            pass
        ...
        logger.log(
            ...
            what=operation_name,
            where="Azure Service",
            why=f"Azure API call failed: {str(e)}",
            status="ERROR",
            balance_before=balance_before["total_balance"]
        )
...
def call_azure_openai_api(prompt):
    """Example Azure OpenAI API call with balance monitoring."""
    return azure_api_call_with_balance_check(
        "azure_openai_completion",
        lambda: azure_openai_client.completions.create(
            model="gpt-4",
            prompt=prompt
        )
    )
...
### Products NOT Covered by Azure Credits

The following products are **NOT** covered by Azure credits and will be charged regardless of credit balance:
...
- **Ubuntu Advantage**
- **Visual Studio Enterprise** (Monthly/Annual)
- **Visual Studio Professional** (Monthly/Annual)
- **Azure Marketplace** products
- **Azure Support Plans**

**IMPORTANT:** Monitor usage of these services separately as they will incur charges even with available credits.

### Integration with Framework

When using Azure services in AI operations:

1. **MANDATORY Credit Monitoring** (Before Every Azure API Call):
   - Check credit balance before operation
   - Log balance in Who/What/Where/When/Why system (Section 14)
   - Store balance in state management (Section 19.2)

2. **MANDATORY Credit Monitoring** (After Every Azure API Call):
   - Check credit balance after operation
   - Calculate balance change
   - Log remaining balance
   - Alert if balance is low (< $10)

3. **Logging Requirements** (Section 14):
   - Log all Azure API calls with balance tracking
   - Include balance_before, balance_after, balance_change
   - Document remaining_balance in all Azure logs

4. **State Management** (Section 19.2):
   - Store credit balance in SSD storage (fast access)
   - Archive balance history to HDD storage
   - Include balance in session context reconstruction

5. **GitHub Integration** (Section 18):
   - Document Azure usage and balance in GitHub issues
   - Create alerts if balance drops below threshold
   - Track credit consumption over time

6. **Performance Monitoring** (Section 19.7):
   - Monitor credit consumption rate
   - Alert on unusual spending patterns
   - Track cost per operation

7. **Security** (Section 3.2):
   - Store Azure credentials securely (never in code)
   - Use Azure Key Vault for sensitive credentials
   - Rotate access tokens regularly


### Unified Credit Manager (All Cloud Providers)

**Centralized credit monitoring for Azure, AWS, and Google Cloud.**

```python
class UnifiedCreditManager:
    """Unified credit monitoring for Azure, AWS, and Google Cloud."""
    
    def __init__(self):
        self.azure = AzureCreditMonitor()
        # self.aws = AWSCreditMonitor()  # Implement when needed
        # self.gcp = GCPCreditMonitor()  # Implement when needed
        self.alert_threshold = 10.0  # Alert if balance < $10
    ...
    def check_all_balances(self):
        ...
        Returns:
            Dictionary with balances for each provider
        """
        balances = {}
        
        try:
            balances["azure"] = self.azure.get_current_balance()
        except Exception as e:
            balances["azure"] = {"error": str(e), "total_balance": 0.0}
        
        # Add AWS and GCP when implemented
        # try:
        #     balances["aws"] = self.aws.get_current_balance()
        # except Exception as e:
        #     balances["aws"] = {"error": str(e), "total_balance": 0.0}
    ...
    def get_total_balance(self):
        ...
        total = sum(
            b.get("total_balance", 0.0) 
            for b in balances.values() 
            if isinstance(b, dict) and "error" not in b
        )
        return total
    
    def log_unified_balance(self, operation_name: str):
        """
        Log consolidated balance across providers.
        ...
        logger.log(
            who="UnifiedCreditManager",
            what="check_all_balances",
            where="Multi-Cloud",
            why=f"Credit monitoring: {operation_name}",
            status="INFO",
            total_balance=total_balance,
            azure_balance=balances.get("azure", {}).get("total_balance", 0.0),
            # aws_balance=balances.get("aws", {}).get("total_balance", 0.0),
            currency="USD"
        )
        
        # Alert if any provider is low
        for provider, balance in balances.items():
            if isinstance(balance, dict) and "error" not in balance:
                provider_balance = balance.get("total_balance", 0.0)
                if provider_balance < self.alert_threshold:
                    logger.log(
                        who="UnifiedCreditManager",
                        what="low_balance_alert",
                        where=f"{provider.upper()}",
                        why=f"Credit monitoring: Low balance warning ({provider})",
                        status="WARNING",
                        provider=provider,
                        remaining_balance=provider_balance
                    )
        
        return balances

# Global instance
unified_credit_manager = UnifiedCreditManager()

# Usage:
def cloud_api_call_with_unified_monitoring(provider: str, operation_name: str, api_function, *args, **kwargs):
    """
    Unified wrapper for cloud API calls with credit monitoring.
    
    Args:
        provider: "azure", "aws", or "gcp"
        operation_name: Name of operation
        api_function: The API function to call
        *args, **kwargs: Arguments for api_function
        
    Returns:
        Result of api_function
    """
    # Check all balances before
    balances_before = unified_credit_manager.check_all_balances()
    ...
    try:
        ...
        balances_after = unified_credit_manager.check_all_balances()
        
        # Log unified balance
        unified_credit_manager.log_unified_balance(operation_name)
        
        return result
        
    except Exception as e:
        # Still log balance even on error
        unified_credit_manager.log_unified_balance(f"{operation_name}_ERROR")
        raise
```
...
```python
# Before: Separate monitoring for each provider
balance_before = azure_credit_monitor.log_balance_before("operation")
result = azure_api_call()
balance_after = azure_credit_monitor.log_balance_after("operation", balance_before)

# After: Unified monitoring
result = cloud_api_call_with_unified_monitoring(
    provider="azure",
    operation_name="azure_openai_completion",
    azure_function=lambda: azure_openai_client.completions.create(model="gpt-4", prompt=prompt)
)
```
...
## Appendix C: AWS Free Tier Account

**Account Name:** MatoTeziTanka  
**Account ID:** 538153606828  
**Service Provider:** Amazon Web Services, Inc.  
**ARN:** arn:aws:account::538153606828:account  
**Plan Type:** Free Tier (Free Plan)  
**Password:** <AWS_PASSWORD> (stored securely)  
**Last Updated:** 2025-11-26

### Account Status

- **Credits Remaining:** $100.00 USD
- **Days Remaining:** 182 days (expires May 27, 2026)
- **Free Period:** Up to 6 months from sign-up
- **Current Month Usage:** Unable to load (check AWS console)
- **Forecasted Month End:** Unable to load (check AWS console)
- **Cost Breakdown:** Unable to load (check AWS console)

### Free Tier Benefits

#### Free Plan (Current)
- **Duration:** Up to 6 months without cost or commitment
- **Credits:** Up to $200 USD in credits
- **Services:** Includes free usage of select services
- **Charges:** No charges incurred unless switching to Paid Plan
- **Scaling:** Workloads scale beyond credit thresholds
- **Access:** Access to all AWS services and features

#### Paid Plan (Available)
- **Credits:** Up to $200 USD in credits
- **Services:** Includes free usage of select services
- **Billing:** Pay beyond credit thresholds
- **Scaling:** Workloads scale beyond credit thresholds
- **Access:** Access to over 150 AWS services and features

### Free Tier Services (187+ Available)

#### Always Free Services (30+)
- **Perpetually free** service offerings with specified monthly limits
- **Credit Application:** Credits automatically applied when exceeding free usage limits
- **Access:** Available on both Free and Paid plans
...
- **Amazon S3** - Secure, durable, and scalable object storage infrastructure
  - Available on both plans
  - Free tier includes limited storage and requests

**Databases:**
- **Amazon RDS** - Managed Relational Database Service (MySQL, PostgreSQL, MariaDB, SQL Server)
  - Available on both plans
  - Free tier includes limited hours per month

- **Amazon DynamoDB** - Serverless, NoSQL, fully managed database
  - Available on both plans
  - Free tier includes limited read/write capacity units

**AI & Machine Learning:**
- **Amazon Bedrock** - Build and scale generative AI applications with foundation models
  - Available on both plans
  - Free tier includes limited inference requests

- **Amazon Bedrock AgentCore** - Build, deploy, and operate agents securely at scale
  - Paid plan exclusive
  - No infrastructure management required

- **Amazon SageMaker AI** - Machine learning for every data scientist and developer
  - Available on both plans
  - Free tier includes limited training hours

**Serverless:**
- **AWS Lambda** - Run code without thinking about servers or clusters
  - Available on both plans
  - Free tier includes limited requests and compute time

**Additional Services:**
- 180+ more free tier services available
- See AWS Free Tier page for complete list

### Usage Monitoring

#### Current Status
- **Credits Remaining:** $100.00 USD
- **Days Remaining:** 182 days (until May 27, 2026)
- **Monitoring:** Check AWS Billing & Cost Management console for:
  - Current month usage
  - Forecasted month end costs
  - Detailed cost breakdown by service

#### Best Practices
- **Monitor Usage:** Regularly check AWS console for usage and costs
- **Set Alerts:** Configure billing alerts to avoid unexpected charges
- **Stay Within Limits:** Use free tier services within specified limits
- **Track Credits:** Monitor credit depletion to avoid service interruption
- **Document Usage:** Log all AWS service usage in framework logs (Section 14)

### Integration with Framework

When using AWS services in AI operations:

1. **Logging Requirements** (Section 14):
   - Log all AWS API calls using Who/What/Where/When/Why logging system
   - Document service usage and costs
   - Track credit consumption

2. **GitHub Integration** (Section 18):
   - Document AWS usage in GitHub issues
   - Track infrastructure changes
   - Version control AWS configurations

3. **LLM Operations** (Section 19):
   - Use AWS Bedrock for cloud-based LLM operations
   - Leverage SageMaker for ML workloads
   - Document AWS usage in state management logs

4. **Storage Operations** (Section 19.2, 21):
   - Use S3 for cloud storage when local VM storage is insufficient
   - Integrate with local SSD/HDD storage strategy
   - Document storage operations

5. **Performance Monitoring** (Section 19.7):
   - Monitor AWS resource usage
   - Track costs and credits
   - Alert on high usage or cost thresholds

6. **Security** (Section 3.2):
...
#### AI & Machine Learning
- Bedrock, SageMaker, Comprehend, Rekognition, Polly, Transcribe

#### Networking
- VPC, CloudFront, Route 53, API Gateway, Direct Connect

#### Security & Identity
- IAM, Cognito, Secrets Manager, KMS, Shield

#### Management & Governance
- CloudWatch, CloudFormation, Systems Manager, Config, Trusted Advisor

### Important Notes

- **Credits Expiration:** Access to AWS services will end when credits are depleted or free period ends
- **Service Interruption:** Plan for service migration if credits run out before May 27, 2026
- **Upgrade Consideration:** Consider Paid Plan if production workloads require extended access
- **Cost Monitoring:** Regularly check AWS console as current month usage may not load in dashboard
- **Security:** Store AWS credentials securely, never in code or repositories

### New Customer Benefits

- **Sign-up Credits:** $100 USD credits at sign-up
- **Additional Credits:** Up to $100 USD more to earn as you explore key AWS services
- **Total Potential:** Up to $200 USD in credits
- **Test Drive:** Free Plan for up to 6 months
- **No Charges:** Won't be charged unless choosing Paid Plan

---

## Appendix D: Google Gemini 3 Pro (Student Access)

**Access Type:** Free Student Access  
**Model:** Gemini 3 Pro  
**Status:** Active  
**Last Updated:** 2025-11-26  
**API Access:** Not included (free tier does not include API access)

### Access Details

- **Eligibility:** Student account
- **Access Method:** Free via student program
- **Model Version:** Gemini 3 Pro
- **API Access:** Not available on free student tier
- **Usage:** Available through Google's web interface or applications that support student accounts

### Limitations

#### API Access
- **Status:** Not included with free student access
- **Note:** Free student tier provides access to Gemini 3 Pro through web interface only
- **API Usage:** Requires paid API access or different subscription tier
- **Alternative:** Use Google Colab Pro for Education (see Appendix C) which includes `google-colab-ai` library for API-free LLM access

#### Usage Restrictions
- **Interface:** Web-based access through Google's interface
- **Rate Limits:** Subject to student tier limitations
- **Features:** Full Gemini 3 Pro capabilities available through web interface
- **API Calls:** Not available without paid API subscription

### Integration with Framework

When using Gemini 3 Pro in AI operations:

1. **Web Interface Usage** (Section 20):
   - Use Gemini 3 Pro for prompt testing and development
   - Leverage for initial concept validation
   - Document prompts and responses in logs (Section 14)

2. **Alternative API Access**:
   - **Google Colab Pro** (Appendix C): Use `google-colab-ai` library for API access
   - **Local VM** (Section 19): Use local LLMs via LM Studio when API access needed
   - **Azure Services** (Appendix B): Consider Azure OpenAI if API access required

3. **Workflow Integration**:
   - Use Gemini 3 Pro web interface for:
     - Initial prompt development
     - Response quality testing
     - Model capability exploration
   - Use other resources (Colab, local VM, Azure) for:
     - Automated API calls
     - Production workloads
     - Integration with code

4. **Logging Requirements** (Section 14):
   - Document all Gemini 3 Pro usage
   - Log prompts and responses
   - Track usage patterns and results

### Comparison with Other LLM Resources

| Resource | API Access | Cost | Best For |
|----------|------------|------|----------|
| **Gemini 3 Pro (Student)** | ❌ No | Free | Web-based testing, prompt development |
| **Google Colab Pro** | ✅ Yes (via library) | Free (Education) | API access, notebooks, GPU acceleration |
| **Local VM (LM Studio)** | ✅ Yes | Free | Full control, no rate limits, persistent sessions |
| **Azure OpenAI** | ✅ Yes | Free tier available | Production workloads, enterprise integration |
| **AWS Bedrock** | ✅ Yes | Free tier available | Cloud-based, scalable |

### Usage Notes

- **Primary Use Case:** Web-based LLM access for development and testing
- **API Alternative:** Use Google Colab Pro for Education (if available) for API access via `google-colab-ai`
- **Production Workloads:** Consider local VM (Section 19) or cloud APIs (Azure/AWS) for automated operations
- **Documentation:** Log all Gemini 3 Pro interactions in framework logs (Section 14)
