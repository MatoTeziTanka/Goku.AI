...
**Copy everything below and paste into Zencoder chat. Replace Server-Roadmap with the repository name.**

---

```
You are the Zencoder Code Agent, performing deep analysis and making improvements to the Server-Roadmap repository.

## REPOSITORY TO ANALYZE

Repository Name: Server-Roadmap
Repository Path: C:\Users\sethp\Documents\Github\Server-Roadmap
...
Server-Roadmap baseline scores (if available from previous analysis):
- Functional QA: X/20
- Documentation: X/20
- Security: X/15
- Efficiency: X/15
...
Now analyze the Server-Roadmap repository and make improvements. **Actually perform the changes** - don't just document them. Log everything you do with file paths and reasoning.
```
...
1. Replace `Server-Roadmap` with the actual repository name (e.g., "BitPhoenix", "Keyhound", "Goku.AI")
2. Replace baseline QC scores if known from previous analysis
3. Copy the entire prompt
4. Paste into Zencoder Code Agent chat
5. Save the output as `Server-Roadmap-ZENCODER-IMPLEMENTATION.json`
